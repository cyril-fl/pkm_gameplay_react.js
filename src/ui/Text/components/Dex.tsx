  ...
  return (
    <>
      ...
      <div className="flex flex-col gap-4">
        {dexEntries &&
          dexEntries.map((entry: DexEntry, index: number) => (
            <div
              key={index}
              className="bg-GameBoy-black p-1 rounded text-GameBoy-white font-jersey-15 text-xl border-1 border-GameBoy-black bg-GameBoy-black"
            >
                ...
                <div className="flex gap-4 items-baseline justify-between">
                  ...
                  <p className="flex gap-4 text-lg self-end ">
                    {entry.types.map((type: type, index: number) => (
                      <span
                        key={index}
                        className="px-3 py-0.5 rounded-md bg-GameBoy-white text-GameBoy-black"
                      >
                        {" "}
                        {type.name}{" "}
                      </span>
